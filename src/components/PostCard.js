  ...
  return (
    <Card className="">
      <div className="flex relative">
        <Link className="cursor-pointer hover:opacity-70" href={"/profile"}>
        ...
        {isOpen && (
          <ClickAwayListener onClickAway={onClickAway}>
            <div className="absolute top-9 bg-white rounded-md border p-3 gap-2 right-1 flex flex-col shadow-lg shadow-purple-200 z-10">
              <a
                onClick={toggleSave}
                className="flex gap-2 py-2 hover:bg-purple-200/50 -mx-2 px-2  hover:cursor-pointer hover:py-2 rounded-md hover:shadow-md transition-all hover:scale-105"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
      ...
      <div>
        ...
        {photos?.length > 0 && (
          <div className="flex gap-4">
            {photos.map((photo) => (
              <div
                className="h-36 w-auto rounded-md overflow-hidden"
                key={photo}
              >
      ...
      <div className="flex mt-3 gap-5">
        <button className="flex gap-2 items-center" onClick={toggleLike}>
          <svg
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className={"w-6 h-6 " + (isLikesByMe ? "fill-purple-500" : "")}
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
