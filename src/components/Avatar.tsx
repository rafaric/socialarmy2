...
const sizeMap: Record<AvatarSize, { wrapper: string; ring: string }> = {
  sm: { wrapper: "w-9 h-9",   ring: "p-[2px]" },
  md: { wrapper: "w-11 h-11", ring: "p-[2px]" },
  lg: { wrapper: "w-20 h-20", ring: "p-[3px]" },
};
...
function Placeholder({ className }: { className: string }) {
  return (
    ...
    >
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-1/2 h-1/2 text-white/60">
...
const Avatar = ({ url, size = "lg", ring = false }: AvatarProps) => {
  ...
  const inner = url ? (
    <img src={url} alt="avatar" className={`${s.wrapper} object-cover rounded-full`} />
  ) : (
